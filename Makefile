...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=helichain \
	-X github.com/cosmos/cosmos-sdk/version.AppName=helichaind \
...
install: validate-go-version go.sum
	@echo "--> Installing helichaind"
	@echo "go install -mod=readonly $(BUILD_FLAGS) ./cmd/helichaind"
	@go install -mod=readonly $(BUILD_FLAGS) ./cmd/helichaind

build: validate-go-version go.sum
	@echo "--> Build helichaind"
	@go build -mod=readonly $(BUILD_FLAGS) -o ./build/helichaind ./cmd/helichaind
...
release:
	...
	$(DOCKER) run \
		...
		-v `pwd`:/go/src/github.com/Heli-Chain/heli \
		-w /go/src/github.com/Heli-Chain/heli \
